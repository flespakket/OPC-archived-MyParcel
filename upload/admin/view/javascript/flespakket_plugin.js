...
//alert("order: " + orderId + "   time:"+timestamp+"   cons:"+consignmentId+"    trac:"+tracktrace_link+"     ret:"+retour);
    	...
    	mypa_img.alt = 'print';
    	mypa_img.src = mano_admin_path+'model/myparcel/images/flespakket_pdf.png';
    	if(retour == 1) mypa_img.src = mano_admin_path+'model/myparcel/images/flespakket_retour.png';
...
function createNewConsignment(orderId, orderPckId, retour)
{
    ...
    popup = window.open(
        'model/flespakket/flespakket_plugin.php?action=post' + '&order_id=' + orderId + '.' + h2.value + '&timestamp=' + timestamp + retourparam,
        'flespakket',
...
function printConsignments(consignmentList)
{
    ...
    popup = window.open(
        'model/flespakket/flespakket_plugin.php?action=print' + '&consignments=' + consignmentList + '&timestamp=' + timestamp,
        'flespakket',
...
function processConsignments(consignmentList)
{
    ...
    popup = window.open(
        'model/flespakket/flespakket_plugin.php?action=process' + '&order_ids=' + consignmentList + '&timestamp=' + timestamp,
        'flespakket',
        'width=415,height=365,dependent,resizable,scrollbars'
        );
    if (window.focus) { popup.focus(); }
